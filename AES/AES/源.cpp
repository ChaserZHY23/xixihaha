...
static unsigned char gUserKey[8] = { 0 };
	...
	int           *pOutDataLen)
{
	...
	rv = myic_DESEncrypt(gUserKey, 8, padDate, (int)padDateLen, pOutData, pOutDataLen);
	if (rv != 0)
	{
		if (padDate != NULL)
		{
	...
	int           *pOutDataLen)
{
	...
	//解密
	rv = myic_DESDecrypt(gUserKey, 8, pInData, nInDataLen, tmpPlain, pOutDataLen);
	if (rv != 0)
	{
...
int sebs_des_update_key(unsigned char* pUpdateKey) {

	if (pUpdateKey == NULL) {
		return -1;
	}
	memcpy(gUserKey, pUpdateKey, 8);
...
void main()
{
	...
	unsigned char key1[8] = { 0xff,0xff,0xff,0xff,0xff,0x12,0x34,0x56 };
	unsigned char uPdk[24] = { 0x84,0xbc,0xa1,0xed,0x9f,0xd6,0xe4,0x95,0x42,0xd7,0x71,0xdd,0xca,0x9a,0x18,0x18,0x56,0x89
		,0x45,0x82,0x5b,0xd2,0x9a,0xd5 };
	DesUpdateKey(key1);
	int PubKeyLen = 0;
	unsigned char uPublicKey[16] = { 0 };
	DesDec(uPdk, sizeof(uPdk), uPublicKey, &PubKeyLen);
	printf("################\n");
	for (i = 0; i <16; i++) {
		printf("%02x ", uPublicKey[i]);
	}
	printf("\n################\n");
