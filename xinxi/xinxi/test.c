...
{
	int i, j;
	for (i = 0; i < 4; i++) w[i] = getWordFromStr(key + i * 4);
	for (i = 4, j = 0; i < 44; i++)
	{
		if (i % 4 == 0)
		{
...
{
	int i, j;
	for (i = 0; i < 4; i++) for (j = 0; j < 4; j++) array[i][j] = getNumFromSBox(array[i][j]);
}
...
static void mixColumns(int array[4][4])
{
	int tempArray[4][4];
	int i, j;
	for (i = 0; i < 4; i++) for (j = 0; j < 4; j++) tempArray[i][j] = array[i][j];
	for (i = 0; i < 4; i++) for (j = 0; j < 4; j++)
	{
...
{
	int i, j;
	for (i = 0; i < 4; i++) for (j = 0; j < 4; j++) array[i][j] = getNumFromS1Box(array[i][j]);
}
...
void writeStrToFile(char *str, int len, char *fileName)
{
	...
	for (int i = 0; i < len; i++) putc(str[i], fp);
	fclose(fp);
}
...
int main(int argc, char const *argv[])
{
	...
	while (1)
	{
		...
		if (klen != 16)
		{
			printf("请设置16个字符的密钥,当前密钥的长度为%d\n", klen);
		}
		else
		{
			printf("你输入的密钥为：%s\n", key);
			printf("切记,如若需要解密,设置的密钥需与加密时的密钥相同！");
			break;
		}
	}
	printf("\n\n");
