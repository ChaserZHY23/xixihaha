...
void main()
{
	...
	while (flag)
	{
		...
		cout << endl << "cd 改变目录 create 创建文件 del删除文件" << endl << "dir 显示目录 md 创建目录 rd 删除目录" << endl << "exit 退出" << endl;
		cout << endl << "-----------------------------------------------------" << endl;
		printf("%s:>#", path);
		gets(string);
		len = strlen(string);
		...
		else
		{
			...
			if ((!strcmp(command, "CD")) || !strcmp(command, "create"))
			{
				comm = 1;
			}
			else
